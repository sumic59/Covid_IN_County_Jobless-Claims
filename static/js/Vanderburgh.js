...
function readUnemploymentData() {
  ...
  //read the json file
  d3.json("static/data/Vanderburgh_Co_unemployment_stats.json").then((sampleNames) => {
    ...
    plotBarChart(xAxisDataOldRecession, yAxisDataOldRecession, "oldbar", "Vanderburgh Unemployment Rate from 2008 - 2014");
    ...
    plotBarChart(xAxisDataNewRecession, yAxisDataNewRecession, "newbar", "Vanderburgh Unemployment Rate from 2015 - Current");

  });
}

function readCovidUempClaimsData() {
  d3.json("static/data/vanderburgh_covid_weekly_claims.json").then((data) => {
